    ...
    return (
        <>
            <div className='grid grid-cols-2 md:grid-cols-4 gap-6 mt-6' >
                {[
                    { name: "Blue Night Hoodie", link: "blue_night_hoodie.png" },
                    { name: "Purple Hoodie", link: "purple_hoodie.png" }
                ].map((p, i) => (
                    <div className='bg-white rounded-xl shadow hover:shadow-lg p-4 cursor-pointer' key={i}>
                        <img src={`./products/hoodie/${p.link}`} alt={p.name} className="w-full h-48" />
